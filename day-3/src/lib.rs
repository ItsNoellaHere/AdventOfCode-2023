pub mod part_1 {
            ...
            for (x, c) in line.chars().enumerate() {
                if c.is_digit(10) {
                    ...
                    if points.last().unwrap_or_default().is_neighbour(&point)
                        && last_part.number != PartNumber::Symbol
                        && last_part.number != PartNumber::None
                    {
    ...
    pub fn number_has_neighbor_symbol(number: &Part, symbols: &Vec<Part>) -> bool {
        number.points.iter().any(|point| point.is_neighbour_of_any(&symbols.iter().flat_map(|s| s.points.clone()).collect()))
    }

    pub fn sum_of_part_numbers(input: &Vec<String>) -> usize {
        let parts = parse_input(input);
        let symbols: Vec<Part> = parts.iter().filter(|p| p.number == PartNumber::Symbol).map(|p| p.clone()).collect();
        parts.iter().filter(|p| p.number != PartNumber::Symbol && number_has_neighbor_symbol(p, &symbols)).map(|p| match p.number {
            PartNumber::Number(n) => n,
            _ => 0,
        }).sum()
    }
...
mod tests {
    mod part_1 {
        ...
        fn single_number_input() {
            ...
            assert_eq!(result.first().unwrap().points.first().unwrap(), &Point::new(2, 0));
            assert_eq!(result.first().unwrap().points.last().unwrap(), &Point::new(4, 0));
        }
        ...
        fn multi_symbol_input() {
            ...
            assert_eq!(result.last().unwrap().points.first().unwrap(), &Point::new(7, 0));
        }
